...
@section('title')
    <title>TA1 | Seminar Penguji</title>
@endsection
@section('content')
    <div class="container">
        @if ($errors->any())
        ...
        @endif
        <div class="card">
            <div class="card-header"><h3>Jadwal Seminar Penguji</h3></div>
            <div class="card-body">
                @if ((count($listOfMahasiswa1) + count($listOfMahasiswa2)) > 0)
                    <form action="/dosen/ta1/seminar/edit_jadwal_seminar" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan data seminar?');">
                        ...
                        <table class="table table-striped" id="seminar-penguji-table">
                            <thead>
                            <th>Kelompok</th>
                            <th>Shift</th>
                            <th>NIM</th>
...
@section('scripts')
    <script>
        $(document).ready(function()
            {
                $("#seminar-penguji-table").dynatable();
            }
        );
        $.when($('#seminar-penguji-table').dynatable()).done(function() {
